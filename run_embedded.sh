...
if [ -z $1 ]; then
    ...
    if [ -z $2 ]; then
        USER_NAME=embedded
        CONTAINER_LABEL=emb_$1
        CONTAINER_ALIAS=$1
    else
        USER_NAME=$2
        CONTAINER_LABEL="emb_$1-$2"
        CONTAINER_ALIAS="$1-$2"
    fi

    # Container paths
    CONTAINER_USER_HOME=/home/$USER
    CONTAINER_WORKDIR=$CONTAINER_USER_HOME/forecast-nucleo
    CONTAINER_SCRIPTS=$CONTAINER_USER_HOME/scripts

    # Host paths
    HOST_USER_HOME=$(pwd)/docker/container/$CONTAINER_ALIAS/home/$USER
    HOST_WORKDIR=$HOST_USER_HOME/forecast-nucleo
    ...
    docker run -it --rm \
        ...
        leggedroboticsusp/legged-ws:emb-$1
